...
public:
    ...
    void setup() {
        ...
        for (int i = 0; i < kBlips; ++i) {
            ...
            ChipSoundNote n{Wave::Square, hz, 0.10f, 0.9f};
            n.attack = 0.002f; n.decay = 0.015f; n.sustain = 0.8f; n.release = 0.03f;
            blips_[i] = n.build();
        }
...
private:
    ...
    static void add(ChipSoundBundle& b, Wave w, int midi, float t, float dur,
        ...
        n.attack = 0.004f; n.decay = 0.02f; n.sustain = 0.9f; n.release = release;
        b.add(n, t);
    }
    ...
    static Sound buildMelody(int n) {
        ...
        switch (n) {
            case 6:  // jackpot: fast rising arpeggio with a sparkly finish
                add(b, Wave::Square,   72, 0.00f, 0.10f, 0.62f);
                add(b, Wave::Square,   76, 0.10f, 0.10f, 0.62f);
                add(b, Wave::Square,   79, 0.20f, 0.10f, 0.62f);
                add(b, Wave::Square,   84, 0.30f, 0.10f, 0.66f);
                add(b, Wave::Triangle, 88, 0.40f, 0.06f, 0.66f);
                add(b, Wave::Square,   84, 0.46f, 0.06f, 0.62f);
                add(b, Wave::Square,   91, 0.54f, 0.30f, 0.68f, 0.18f);
                break;
            case 5:  // nice: confident rising third
                add(b, Wave::Triangle, 71, 0.00f, 0.14f, 0.85f);
                add(b, Wave::Triangle, 74, 0.16f, 0.14f, 0.85f);
                add(b, Wave::Square,   79, 0.32f, 0.26f, 0.82f, 0.14f);
                break;
            case 4:  // ok: a small two-note lift
                add(b, Wave::Square,   67, 0.00f, 0.16f, 0.82f);
                add(b, Wave::Square,   72, 0.18f, 0.26f, 0.85f, 0.14f);
                break;
            case 3:  // plain: two flat notes
                add(b, Wave::Triangle, 69, 0.00f, 0.18f, 0.85f);
                add(b, Wave::Triangle, 69, 0.22f, 0.20f, 0.82f);
                break;
            case 2:  // meh: a half-step sag
                add(b, Wave::Square,   64, 0.00f, 0.18f, 0.80f);
                add(b, Wave::Square,   61, 0.22f, 0.26f, 0.80f, 0.12f);
                break;
            case 1:  // sad: slow low fall, "womp womp"
                add(b, Wave::Sawtooth, 53, 0.00f, 0.30f, 0.85f, 0.10f);
                add(b, Wave::Sawtooth, 48, 0.34f, 0.55f, 0.88f, 0.22f);
                break;
        }
        return b.build();
    }
